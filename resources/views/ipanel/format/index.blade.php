            ...
            <div class="portlet light bordered">
                ...
                <div class="portlet-body">
                    ...
                    <table class="table table-striped table-bordered table-hover table-checkable order-column">
                        ...
                        <tbody>
                        @if($pending)
                            @foreach($pending as $format)
                                <tr class="odd gradeX">

                                    <td><strong>{{ $format->noun }}</strong></td>
                                    <td><strong>{{ $format->verb }}</strong></td>
                                    <td><strong>{{ $format->adjective }}</strong></td>
                                    <td><strong>{{ $format->adverb }}</strong></td>
                                    ...
                                    <td width="10%">
                                        ...
                                        @endphp
                                        <strong>{{ ($format->added_by != null) ? @$user->username : '-' }}</strong>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        ...
                                        @if(Auth::user()->roles[0]->id != 3)
<a href="{{ url('ipanel/format/status/pending',$format->id) }}" class="btn btn-icon-only blue">
        <i class="fa fa-check"></i>
    </a>
<button class="btn btn-icon-only red " type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal"  data-whatever="{{$format->id}}"><i class="fa fa-remove"></i></button>
        ...
        <div class="col-md-12">
            ...
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        <i class="icon-settings font-dark"></i>
                        <span class="caption-subject bold uppercase"> قائمة شكل الكلمة </span>
                    </div>
                    <div class="actions">
                        <form action="{{ url('/ipanel/format/search') }}" method="get">
                ...
                <div class="portlet-body">
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('format.create')) }}" class="btn sbold green"> إضافة
                                        جديد
                            ...
                            @foreach($formats as $format)
                                <tr class="odd gradeX">
                                    ...
                                    <td class="text-center">
                                        ...
                                        @if(Auth::user()->roles[0]->id != 3)
                                            <a href="{{ url('ipanel/format/'.$format->id) }}"
                                               class="btn btn-icon-only blue">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            <button class="btn btn-icon-only red delete-format"
                                                    data-id="{{ $format->id }}">
      ...
      <div class="modal-header">
        ...
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
              ...
              <form  method="Post" action="{{ route('format.status') }}">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('.delete-format').click(function () {
                ...
                //alert('clicked..' + id)
                var id = $(this).closest(".delete-format").attr("data-id");
                swal({
                        title: "حذف العنصر!",
                    ...
                    function () {
                        ...
                        $
                        $deleteItem = '{{ url('ipanel/format/destroy') }}' + '/' + id;
                        setTimeout(function () {
